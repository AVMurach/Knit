...
function GetOutlets(searchText) {
	...
	var q = new Query();
	var qtext = "SELECT C.Id, C.Description, CC.Description AS Address " +
			"FROM Catalog_Clients C " +
			"LEFT JOIN Catalog_Clients_ContactInfo CC ON CC.Ref = C.Id AND CC.Form == @adr " +
			"WHERE (1 = 1) ";
	if (searchText != null && searchText != ""){
		searchtail = "AND  Contains(C.Description, @SearchText)";
		
		q.AddParameter("SearchText", searchText);
		qtext = qtext + searchtail;
	}
	q.Text = qtext + "  ORDER BY C.Description";

	q.AddParameter("adr", "@ref[Catalog_FormsContactsInfo]:dad59784-79a7-4c73-a0d1-617941d288b2"); //Àäðåñ
...
function GetOutletParameters(outlet) {
	...
	query.Text = "SELECT P.Id, P.Description, P.DataType, DT.Description AS TypeDescription, OP.Id AS ParameterValue, OP.Value, P.Visible, P.Editable " +
			...
			"ELSE CASE " +
			"WHEN OP.Value IS NULL OR RTRIM(OP.Value)='' THEN 'â€”' " +
...
function ChooseDateTime(entity, attribute, control, func, title) {
	...
	if (String.IsNullOrEmpty(startKey) || startKey=="â€”")
...
function ChooseBool(entity, attribute, control, func, title) {
	...
	var listChoice = [[ "â€”", "-" ], [Translate["#YES#"], Translate["#YES#"]], [Translate["#NO#"], Translate["#NO#"]]];
	if (func == null)
		func = CallBack;
